...
const questions = [
    ...
    },
    {
        id: 15,
        axis: 'Hiperatividade',
        text: 'Com que frequência você responde perguntas antes que elas sejam concluídas ou tem dificuldade em aguardar sua ver?',
        weight: 1
    }
];
...
const DOM = {
    ...
    dynamicPixArea: document.getElementById('dynamic-pix-area'),
    pixQrImg: document.getElementById('pix-qr-img'),
    pixInput: document.getElementById('pix-input'),
...
async function generatePix() {
    // UI Loading State
    DOM.generatePixBtn.disabled = true;
    DOM.generatePixBtn.textContent = "Gerando PIX...";
    DOM.generatePixBtn.classList.add('loading');
    ...
    try {
        ...
        if (data.id) {
            state.currentPaymentId = data.id;

            // Show dynamic area
            DOM.pixQrImg.src = `data:image/png;base64,${data.qr_code_base64}`;
...
function copyPixToClipboard() {
    DOM.pixInput.select();
    DOM.pixInput.setSelectionRange(0, 99999);
    navigator.clipboard.writeText(DOM.pixInput.value);

    const originalText = DOM.copyPixBtn.textContent;
    DOM.copyPixBtn.textContent = "Copiado!";
    DOM.copyPixBtn.style.backgroundColor = "var(--accent-hover)";

    setTimeout(() => {
        DOM.copyPixBtn.textContent = originalText;
        DOM.copyPixBtn.style.backgroundColor = "";
    }, 2000);
}

// --- 7. SCORING & RESULTS LOGIC ---
...
function init() {
    ...
    // Check for success param (optional fallback)
    ...
    // Mercado Pago Event Listeners
    if (DOM.generatePixBtn) {
        DOM.generatePixBtn.addEventListener('click', generatePix);
    }

    if (DOM.copyPixBtn) {
        DOM.copyPixBtn.addEventListener('click', copyPixToClipboard);
    }
